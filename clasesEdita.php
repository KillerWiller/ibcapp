						...
						<form id="claseForm" method="POST" enctype="multipart/form-data" autocomplete="off">
											...
											<?php 	$data =  json_decode($profesor->listaProfesores());
													...
													if (count($data->JProfesores)>0) {
														foreach ($data->JProfesores as $idx => $stand) {
															// Output a row
															echo "<option value='$stand->Id_Profesor'>" .strtoupper($stand->Nombres_Profesor ." " .$stand->ApePat_Profesor ." " .$stand->ApeMat_Profesor) ."</option>";
														}
													} 										 
											?>
											</select>
							...
							<div class="row g-2">
								<div class="col text-center">
									<span id="spin"></span>
									<span id='state'><input type="submit" id="agregar" class="btn btn-primary btn-lg" value="Guardar" onClick="validar();"></span>
...
<script>
	...
	function validar() {
		...
		if(periodo.value<1){
			periodo.setCustomValidity('Seleccione un periodo');
		}
		if(materia.value<1){
