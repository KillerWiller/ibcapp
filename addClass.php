...
<script>
    ...
    (function($) {
        ...
        $.fn.borraAlumnoCurso = function(idAlumnoCurso,IdSede,IdCurso,anio) {
                var proceed = confirm("¿Desea quitar a este alumno del curso?");
                if (proceed) {
                    $.ajax({
                        url:"ajaxCalls.php",
                        method:"POST",
                        //agregaAlumnoCurso ES UN FLAG PARA SABER A QUE FUNCION LLAMAR EN LA CLASE PHP
                        data:{AJXfn:'borraAlumnoCurso',AJXIidAlumnoCurso:idAlumnoCurso,AJXSede:IdSede,AJXCurso:IdCurso,AJXAnio:anio},
                        success:function(data)
                        {
                            if(data.length > 0){
                                $('#lista').html(data);
                                ...
                                var sAlert = "<div class='alert alert-warning alert-dismissible fade show' role='alert'><svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-exclamation-triangle-fill' viewBox='0 0 16 16'>				<path d='M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z'/>	</svg>		<strong>&nbsp;Atención!</strong> No se pudo eliminar este alumno.<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button></div>";
                                $('#alerta').html(sAlert);
                            }
    ...
    //PARA ELIMINAR ALUMNO DEL CURSO
    function eliminaACurso(clicked_id){
        var curso = $('#curso').val();
        var sede = $('#sede').val();
        var anio = $('#anio').val();
        $(this).borraAlumnoCurso(clicked_id,sede,curso,anio);
    }
    //*****************************************************************************
</script>